...
data PRef
  = -- Source offset, referenced box
    BoxR Int Int Int
  | -- Source offset, referenced line
    LineR Int Int

data PEntry
  = -- Brace token, content
    BoxP Tok [PEntry]
  | -- Line no. token, line no., formula,
    -- rule token, references
    LineP Tok Int Form Tok [PRef]
...
getB :: Int -> [PEntry] -> Result (Form, Form)
